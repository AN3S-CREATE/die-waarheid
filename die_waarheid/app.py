...
def setup_page():
    ...
    st.set_page_config(page_title=f"{APP_NAME} v{APP_VERSION}", page_icon="🕵️", layout="wide", initial_sidebar_state="expanded")
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        page = st.radio(
            ...
            [
                "🏠 Home",
                "🔄 Pipeline Results",  # NEW: Automated pipeline with chronological table
                "📥 Data Import",
                "🎙️ Transcribe Audio",
                "🎙️ Speaker Training",
                "🎙️ Audio Analysis",
...
def page_home():
    ...
    with col2:
        st.metric(
            "Audio Files", audio_files_count, f"Across {len([d for d in AUDIO_DIR.iterdir() if d.is_dir()])} directories"
        )
    ...
    with tab2:
        ...
        with col1:
            ...
            chat_files = st.file_uploader(
                "Select WhatsApp chat export files (.txt)", type=['txt'], accept_multiple_files=True, key="chat_files_primary"
            )
            ...
            if st.session_state.get('show_alternative_upload', False):
                ...
                uploaded_file = st.file_uploader("Upload one file at a time", type=['txt'], key="chat_file_single")
            ...
            if chat_files:
                st.info(
                    f"📁 Processing {len(chat_files)} file(s): {', '.join([f.name for f in chat_files[:3]])}{'...' if len(chat_files) > 3 else ''}"
                )
        ...
        with col2:
            ...
            audio_files = st.file_uploader(
                "Select audio files", type=['mp3', 'wav', 'opus', 'ogg', 'm4a', 'aac'], accept_multiple_files=True
            )
            ...
            if audio_files:
                ...
                if total_size > max_total_size:
                    ...
                    with st.spinner(f"Saving {len(audio_files)} audio files..."):
                        ...
                        if saved_files:
                            st.success(
                                f"✅ Successfully saved {len(saved_files)} audio files ({sum(Path(f).stat().st_size for f in saved_files) / (1024*1024):.1f}MB)"
                            )
    ...
    **Main Function**: Transcribe your voice notes to readable text using Whisper AI.
    ...
    with col1:
        ...
        language = st.selectbox(
            "Language",
            [("af", "Afrikaans"), ("en", "English"), ("nl", "Dutch"), ("auto", "Auto-detect")],
            index=0,
    ...
    with col2:
        ...
        if batch_process:
            ...
            audio_file = st.file_uploader(
                "Upload audio file to transcribe", type=['mp3', 'wav', 'opus', 'ogg', 'm4a', 'aac'], key="transcribe_single"
            )
...
def page_audio_analysis():
    ...
    with col1:
        ...
        whisper_model = st.selectbox("Whisper Model Size", ["tiny", "base", "small", "medium", "large"], index=3)

        language = st.selectbox("Language", ["af", "en", "es", "fr", "de"], index=0)
...
def page_report_generation():
    ...
    with col2:
        ...
        export_formats = st.multiselect("Export Formats", ["Markdown", "HTML", "JSON", "PDF"], default=["Markdown", "HTML"])
    ...
    if len(participants) < 2:
        ...
        for profile in participants:
            with st.expander(f"👤 {profile.primary_username} ({profile.assigned_role.value})"):
                ...
                st.write(
                    f"**Alternate Usernames:** {', '.join(profile.alternate_usernames) if profile.alternate_usernames else 'None'}"
                )
                        ...
                        with st.spinner(f"Training {participant_a.primary_username}..."):
                            try:
                                ...
                                participant_id = system.register_voice_note(
                                    participant_a.primary_username, temp_path, datetime.now()
                                )
                    ...
                    if st.button(f"🎙️ Train {participant_b.primary_username}", key="train_b"):
                        with st.spinner(f"Training {participant_b.primary_username}..."):
                            try:
                                ...
                                # Register voice note
                                participant_id = system.register_voice_note(
                                    participant_b.primary_username, temp_path, datetime.now()
                                )
            ...
            if st.button("🔍 Test Speaker Identification"):
                with st.spinner("Analyzing voice..."):
                    try:
                        ...
                        # Test identification
                        participant_id, confidence = system.identify_speaker("test_user", audio_file=temp_path)
...
def page_settings():
    ...
    with tab2:
        ...
        directories = {"Audio": str(AUDIO_DIR), "Text": str(TEXT_DIR), "Temp": str(TEMP_DIR), "Reports": str(REPORTS_DIR)}
