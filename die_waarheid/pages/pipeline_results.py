...
with st.sidebar:
    ...
    uploaded_files = st.file_uploader(
        "Upload Voice Notes", type=['mp3', 'wav', 'opus', 'ogg', 'm4a', 'aac'], accept_multiple_files=True
    )
    ...
    language = st.selectbox(
        "Language", ["af", "en", "nl"], format_func=lambda x: {"af": "Afrikaans", "en": "English", "nl": "Dutch"}[x]
    )

    model_size = st.selectbox("Whisper Model", ["tiny", "small", "medium", "large"], index=1)
        ...
        with st.spinner("Processing through pipeline..."):
            ...
            if files_to_process:
                ...
                for i, file_path in enumerate(files_to_process):
                    ...
                    result = st.session_state.pipeline.process_voice_note(file_path, language=language, model_size=model_size)
...
if st.session_state.results_df is not None and len(st.session_state.results_df) > 0:
    ...
    with col2:
        st.metric("High Risk", stats.get('high_risk_count', 0), delta=None if stats.get('high_risk_count', 0) == 0 else "⚠️")
    with col3:
        st.metric("Deception", stats.get('deception_count', 0), delta=None if stats.get('deception_count', 0) == 0 else "🚨")
    with col4:
        st.metric(
            "Gaslighting", stats.get('gaslighting_count', 0), delta=None if stats.get('gaslighting_count', 0) == 0 else "⚠️"
        )
    with col5:
    ...
    if show_deception_only:
        filtered_df = filtered_df[
            filtered_df['deception_indicators'].apply(lambda x: len(x) > 0 if isinstance(x, list) else False)
        ]
    if min_stress > 0:
    ...
    **All results appear in one chronological table** - no clicking between pages!
    ...
    st.image(
        "https://via.placeholder.com/1200x400/f0f2f6/333333?text=Chronological+Results+Table+with+All+Analytics",
