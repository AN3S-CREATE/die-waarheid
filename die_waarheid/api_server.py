...
app = FastAPI(title="Die Waarheid API", description="Forensic Analysis API for WhatsApp Communications", version="1.0.0")
...
async def validate_file_security_and_size(file: UploadFile) -> None:
    ...
    if len(content) > MAX_FILE_SIZE:
        raise HTTPException(status_code=413, detail=f"File too large. Maximum size: {MAX_FILE_SIZE/(1024*1024):.0f}MB")
...
async def health_check(request: Request):
    ...
    try:
        from src.gpu_manager import get_optimal_device, gpu_manager, is_gpu_available
        ...
        health_info["gpu"] = {"available": False, "optimization_enabled": False, "error": str(e)}
...
async def security_status(request: Request, api_key: str = Depends(verify_api_key)):
    ...
    if not ADVANCED_SECURITY_AVAILABLE:
        return {"security_enabled": False, "message": "Advanced security module not available"}

    try:
        security_report = security_validator.get_security_report()
        return {"security_enabled": True, "timestamp": datetime.now().isoformat(), **security_report}
    except Exception as e:
...
async def gpu_status(request: Request, api_key: str = Depends(verify_api_key)):
    ...
    """
    try:
        from src.gpu_manager import gpu_manager, is_gpu_available
        ...
        raise HTTPException(status_code=500, detail="Error retrieving GPU status")
    ...
    try:
        ...
        try:
            result = await asyncio.wait_for(
                loop.run_in_executor(executor, transcriber.transcribe, tmp_path, language), timeout=300  # 5 minutes timeout
            )
        except asyncio.TimeoutError:
            raise HTTPException(status_code=504, detail="Transcription timeout after 5 minutes")
...
@limiter.limit("10/minute")
async def analyze_audio(request: Request, file: UploadFile = File(...), api_key: str = Depends(verify_api_key)):
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(
            delete=False, suffix=Path(file.filename).suffix if file.filename else ".tmp"
        ) as tmp_file:
            tmp_path = Path(tmp_file.name)
            content = await file.read()
            tmp_file.write(content)
...
async def initialize_speakers(request: Request, data: dict, api_key: str = Depends(verify_api_key)):
    ...
    try:
        ...
        participant_a_id, participant_b_id = speaker_system.initialize_investigation(participant_a, participant_b)
...
async def train_speaker(
    request: Request, file: UploadFile = File(...), participant_id: str = Form(...), api_key: str = Depends(verify_api_key)
):
    ...
    try:
        ...
        result_id = speaker_system.register_voice_note(participant.primary_username, tmp_path, datetime.now())

        # Clean up
        tmp_path.unlink()

        return {"success": True, "message": f"Voice sample trained successfully", "participant_id": result_id}
