...
class GPUManager:
    ...
    def _check_nvidia_ml(self) -> None:
        ...
        try:
            ...
            if self._cuda_info:
                ...
                self._cuda_info = CUDAInfo(
                    available=len(devices) > 0 and not FORCE_CPU, device_count=len(devices), devices=devices
                )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if devices and not self._cuda_info:
                    self._cuda_info = CUDAInfo(available=not FORCE_CPU, device_count=len(devices), devices=devices)
    ...
    def is_gpu_available(self) -> bool:
        ...
        return (
            ENABLE_GPU
            and not FORCE_CPU
            and self._cuda_info is not None
            and self._cuda_info.available
            and self._cuda_info.device_count > 0
        )
    ...
    def get_optimal_device(self) -> str:
        ...
        best_device = min(self._cuda_info.devices, key=lambda d: d.memory_used / d.memory_total)
    ...
    def optimize_model_loading(self, model_size: str = "small") -> Dict[str, Any]:
        ...
        """
        settings = {"device": "cpu", "fp16": False, "memory_efficient": True, "batch_size": 1, "num_workers": 1}
        ...
        if available_memory > required_memory * 1.5:  # 50% safety margin
            ...
            logger.warning(
                f"Insufficient GPU memory for {model_size} model ({required_memory}MB required, {available_memory}MB available)"
            )
        ...
        if initial_stats["devices"] and final_stats["devices"]:
            for i, (initial_device, final_device) in enumerate(zip(initial_stats["devices"], final_stats["devices"])):
                monitoring_results["changes"][f"device_{i}"] = {
                    ...
                    "utilization_change_percent": final_device["gpu_utilization_percent"]
                    - initial_device["gpu_utilization_percent"],
                }
