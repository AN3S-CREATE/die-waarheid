...
ALLOWED_HTML_TAGS = ['p', 'br', 'strong', 'em', 'u', 'i', 'b', 'ul', 'ol', 'li', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6']

ALLOWED_HTML_ATTRIBUTES = {'*': ['class', 'id'], 'a': ['href', 'title'], 'img': ['src', 'alt', 'width', 'height']}
...
class SecurityValidator:
    ...
    def sanitize_text_input(
        self, text: str, max_length: Optional[int] = None, allow_html: bool = False, strict_mode: bool = True
    ) -> str:
        """
        ...
        if len(text) > max_len:
            raise HTTPException(status_code=413, detail=f"Input too long. Maximum length: {max_len}")
        ...
        # HTML sanitization
        if allow_html:
            text = bleach.clean(text, tags=ALLOWED_HTML_TAGS, attributes=ALLOWED_HTML_ATTRIBUTES, strip=True)
        else:
    ...
    def check_rate_limit(
        self, client_ip: str, endpoint: str, requests_limit: Optional[int] = None, window_seconds: Optional[int] = None
        ...
        _rate_limit_storage[key] = [req_time for req_time in _rate_limit_storage[key] if current_time - req_time < window]

        # Check if limit exceeded
        if len(_rate_limit_storage[key]) >= limit:
            self._log_security_event(
                "rate_limit_exceeded",
                {"client_ip": client_ip, "endpoint": endpoint, "requests": len(_rate_limit_storage[key])},
            )
        ...
        return True, {"requests_made": len(_rate_limit_storage[key]), "requests_limit": limit, "window_seconds": window}
...
def validate_request_security(request: Request) -> None:
    ...
    if not is_allowed:
        raise HTTPException(
            status_code=429, detail="Rate limit exceeded", headers={"Retry-After": str(rate_info.get("retry_after", 3600))}
        )
...
def sanitize_user_input(text: str, max_length: int = 10000) -> str:
    ...
    return security_validator.sanitize_text_input(text, max_length=max_length, allow_html=False, strict_mode=True)


def sanitize_html_content(html_content: str, max_length: int = 50000) -> str:
    """Sanitize HTML content allowing safe tags"""
    return security_validator.sanitize_text_input(html_content, max_length=max_length, allow_html=True, strict_mode=True)


def validate_file_security(content: bytes, filename: str, allowed_types: Optional[List[str]] = None) -> None:
    """
    ...
    """
    is_valid, error_msg = security_validator.validate_file_upload(content, filename, allowed_types)
