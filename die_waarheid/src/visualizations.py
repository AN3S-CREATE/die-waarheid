...
from config import PLOTLY_THEME, SPEAKER_COLORS, STRESS_HEATMAP_COLORSCALE
...
class VisualizationEngine:
    ...
    def create_stress_timeline(self, timeline_data: pd.DataFrame) -> go.Figure:
        ...
        try:
            ...
            fig.add_hline(y=50, line_dash="dash", line_color="orange", annotation_text="High Stress Threshold")
    ...
    def create_speaker_distribution(self, timeline_data: pd.DataFrame) -> go.Figure:
        ...
        try:
            ...
            fig = go.Figure(
                data=[
                    go.Pie(
                        ...
                        marker=dict(
                            colors=[
                                SPEAKER_COLORS.get(f"SPEAKER_{i:02d}", f"#{i*100:06x}") for i in range(len(speaker_counts))
                            ]
                        ),
            ...
            fig.update_layout(title='Message Distribution by Speaker', template=self.theme, height=500)
    ...
    def create_emotion_distribution(self, timeline_data: pd.DataFrame) -> go.Figure:
        ...
        try:
            ...
            fig.update_layout(
                title='Emotion Distribution', xaxis_title='Emotion', yaxis_title='Frequency', template=self.theme, height=500
            )
    ...
    def create_multi_signal_chart(self, timeline_data: pd.DataFrame) -> go.Figure:
        ...
        try:
            ...
            fig.add_trace(
                go.Scatter(
                    x=range(len(audio_data)),
                    y=audio_data['Pitch_Volatility'],
                    name='Pitch Volatility',
            ...
            fig.add_trace(
                go.Scatter(
                    x=range(len(audio_data)), y=audio_data['Silence_Ratio'], name='Silence Ratio', line=dict(color='#4ECDC4')
                ),
                row=2,
                col=1,
            )
            ...
            fig.add_trace(
                go.Scatter(
                    x=range(len(audio_data)), y=audio_data['Intensity_Max'], name='Intensity (Max)', line=dict(color='#FFE66D')
                ),
                row=3,
                col=1,
            )
            ...
            fig.update_layout(title='Multi-Signal Bio-Analysis', template=self.theme, height=800, hovermode='x unified')
    ...
    def create_dashboard(self, timeline_data: pd.DataFrame) -> go.Figure:
        ...
        try:
            fig = make_subplots(
                rows=2,
                cols=2,
                subplot_titles=('Stress Timeline', 'Speaker Distribution', 'Daily Messages', 'Emotion Distribution'),
            ...
            if not audio_data.empty:
                fig.add_trace(
                    go.Scatter(
                        x=audio_data['Recorded_At'], y=audio_data['Stress_Level'], name='Stress', line=dict(color='#FF6B6B')
                    ),
                    row=1,
                    col=1,
                )
            ...
            speaker_counts = timeline_data['Sender'].value_counts()
            fig.add_trace(go.Pie(labels=speaker_counts.index, values=speaker_counts.values, name='Speakers'), row=1, col=2)

            timeline_data['Date'] = pd.to_datetime(timeline_data['Recorded_At']).dt.date
            daily_counts = timeline_data.groupby('Date').size()
            fig.add_trace(
                go.Bar(x=daily_counts.index, y=daily_counts.values, name='Messages', marker=dict(color='#4ECDC4')),
                row=2,
                col=1,
            )
            ...
            if not emotions.empty:
                fig.add_trace(
                    go.Bar(x=emotions.index, y=emotions.values, name='Emotions', marker=dict(color='#FFE66D')), row=2, col=2
                )

            fig.update_layout(title='Forensic Analysis Dashboard', template=self.theme, height=800, showlegend=False)
