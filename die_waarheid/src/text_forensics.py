...
class PatternChangeDetector:
    ...
    def analyze_writing_patterns(self, messages: List[TextMessage], sender: str) -> List[ForensicFinding]:
        """
    ...
    def _analyze_vocabulary_changes(self, messages: List[TextMessage]) -> List[ForensicFinding]:
    ...
    def _analyze_length_patterns(self, messages: List[TextMessage]) -> List[ForensicFinding]:
    ...
    def _analyze_style_changes(self, messages: List[TextMessage]) -> List[ForensicFinding]:
    ...
    def _analyze_timing_patterns(
        self, sender_messages: List[TextMessage], all_messages: List[TextMessage]
...
class StoryFlowAnalyzer:
    ...
    def analyze_story_flow(self, messages: List[TextMessage], topic: str = "") -> List[ForensicFinding]:
        """
    ...
    def _extract_narrative_messages(self, messages: List[TextMessage]) -> List[TextMessage]:
    ...
    def _analyze_timeline_consistency(self, messages: List[TextMessage]) -> List[ForensicFinding]:
        ...
        for msg in messages:
            for pattern, ref_type in time_patterns:
                ...
                if match:
                    time_refs.append({'message': msg, 'type': ref_type, 'value': match.group(0), 'timestamp': msg.timestamp})
    ...
    def _analyze_logical_flow(self, messages: List[TextMessage]) -> List[ForensicFinding]:
                    ...
                    if (neg1 and pos2) or (pos1 and neg2):
                        ...
                        findings.append(
                            ForensicFinding(
                                ...
                                evidence=[f"Message 1: {msg1.text[:100]}...", f"Message 2: {msg2.text[:100]}..."],
    ...
    def _ai_story_analysis(self, messages: List[TextMessage], topic: str) -> List[ForensicFinding]:
...
class ContradictionDetector:
    ...
    def detect_contradictions(self, messages: List[TextMessage], sender: Optional[str] = None) -> List[ForensicFinding]:
        """
    ...
    def _group_by_topic(self, messages: List[TextMessage]) -> Dict[str, List[TextMessage]]:
    ...
    def _check_topic_contradictions(self, topic: str, messages: List[TextMessage]) -> List[ForensicFinding]:
        ...
        for msg in messages:
            ...
            for sentence in sentences:
                if len(sentence.strip()) > 10:
                    claims.append({'message': msg, 'claim': sentence.strip()})
        ...
        for i, claim1 in enumerate(claims):
            for claim2 in claims[i + 1 :]:
                if claim1['message'].sender == claim2['message'].sender:
                    ...
                    similarity = self._check_contradiction_patterns(claim1['claim'], claim2['claim'])
                    ...
                    if similarity > 0.7:
                        ...
                        findings.append(
                            ForensicFinding(
                                ...
                                description="Statements that may contradict each other.",
                                evidence=[f"Claim 1: {claim1['claim'][:100]}", f"Claim 2: {claim2['claim'][:100]}"],
                                message_ids=[claim1['message'].message_id, claim2['message'].message_id],
    ...
    def _ai_contradiction_analysis(self, messages: List[TextMessage]) -> List[ForensicFinding]:
...
{{
    ...
    "consistency_score": 0-100
                ...
                response = self.model.generate_content(
                    prompt, generation_config=genai.types.GenerationConfig(temperature=0.1, max_output_tokens=2000)
                )
                    ...
                    for contradiction in result.get('contradictions', []):
                        ...
                        findings.append(
                            ForensicFinding(
                                finding_id=f"CD_{self.finding_counter:04d}",
                                analysis_type=AnalysisType.CONTRADICTION,
                                severity=severity_map.get(contradiction.get('severity', 'medium'), SeverityLevel.MEDIUM),
...
class PsychologicalAnalyzer:
    ...
    def analyze_psychological_patterns(
        self, messages: List[TextMessage], sender: Optional[str] = None
    ) -> List[ForensicFinding]:
        """
    ...
    def _detect_gaslighting(self, messages: List[TextMessage]) -> List[ForensicFinding]:
        ...
        for msg in messages:
            ...
            for pattern in gaslighting_phrases:
                if re.search(pattern, text_lower):
                    gaslighting_instances.append({'message': msg, 'pattern': pattern})
            ...
            if score >= GASLIGHTING_THRESHOLD:
                ...
                findings.append(
                    ForensicFinding(
                        ...
                        evidence=[f"{g['message'].sender}: {g['message'].text[:100]}" for g in gaslighting_instances[:5]],
    ...
    def _detect_manipulation(self, messages: List[TextMessage]) -> List[ForensicFinding]:
            ...
            if len(instances) >= 2:
                ...
                findings.append(
                    ForensicFinding(
                        ...
                        evidence=[f"{m.sender}: {m.text[:80]}..." for m in instances[:3]],
    ...
    def _analyze_emotional_patterns(self, messages: List[TextMessage]) -> List[ForensicFinding]:
        ...
        for msg in messages:
            ...
            if msg_emotions:
                emotion_timeline.append({'message': msg, 'emotions': msg_emotions})
            ...
            if anger_count >= 3 or fear_count >= 3:
                ...
                findings.append(
                    ForensicFinding(
                        ...
                        evidence=[f"{e['message'].sender}: {', '.join(e['emotions'])}" for e in emotion_timeline[:5]],
    ...
    def _ai_psychological_analysis(self, messages: List[TextMessage]) -> List[ForensicFinding]:
...
class TextForensicsEngine:
    ...
    def analyze_conversation(self, messages: List[TextMessage], topic: str = "") -> ForensicReport:
        """
        ...
        date_range = {'start': min(timestamps).isoformat(), 'end': max(timestamps).isoformat()}
    ...
    def _generate_summary(
        self, messages: List[TextMessage], senders: List[str], findings: List[ForensicFinding]
    ) -> Dict[str, Any]:
    ...
    def _get_recommendations(self, risk_level: str, findings: List[ForensicFinding]) -> List[str]:
        ...
        has_gaslighting = any('gaslighting' in f.title.lower() for f in findings)
        has_contradictions = any(f.analysis_type == AnalysisType.CONTRADICTION for f in findings)
        has_manipulation = any('manipulation' in f.title.lower() for f in findings)
