...
import psutil
from config import APP_NAME, APP_VERSION, AUDIO_DIR, REPORTS_DIR, TEMP_DIR, TEXT_DIR
...
class HealthChecker:
    ...
    def _check_directories(self) -> Dict[str, Any]:
        ...
        directories = {'audio': AUDIO_DIR, 'text': TEXT_DIR, 'temp': TEMP_DIR, 'reports': REPORTS_DIR}
            ...
            try:
                if path.exists():
                    if path.is_dir():
                        ...
                        results[name] = {'status': 'healthy', 'path': str(path), 'file_count': file_count}
                    else:
                        results[name] = {'status': 'error', 'path': str(path), 'error': 'Path is not a directory'}
                        all_healthy = False
                else:
                    results[name] = {'status': 'warning', 'path': str(path), 'message': 'Directory does not exist'}
                    # Create missing directories
                    path.mkdir(parents=True, exist_ok=True)
                    results[name]['message'] = 'Directory created'
            except Exception as e:
                results[name] = {'status': 'error', 'path': str(path), 'error': str(e)}
                all_healthy = False

        return {'status': 'healthy' if all_healthy else 'unhealthy', 'directories': results}
    ...
    def _check_api_connectivity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if analyzer.configured:
                ...
                return {'status': 'warning', 'gemini_configured': False, 'message': 'Gemini API not configured'}
        except Exception as e:
            return {'status': 'error', 'error': str(e)}
    ...
    def get_recent_incidents(self, hours: int = 24) -> list:
        ...
        return [incident for incident in self.incidents if datetime.fromisoformat(incident['timestamp']).timestamp() > cutoff]
...
def setup_monitoring():
    ...
    if health['status'] != 'healthy':
        health_checker.log_incident('warning', f"System started with {health['status']} status", health['checks'])
