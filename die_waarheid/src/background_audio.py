...
class BackgroundAudioAnalyzer:
    ...
    def _classify_noise_type(self, freqs: np.ndarray, magnitude: np.ndarray, sr: int) -> Optional[BackgroundNoiseProfile]:
        ...
        for noise_type, profile in self.noise_profiles.items():
            score = self._match_noise_profile(
                spectral_centroid, spectral_rolloff, spectral_bandwidth, profile, freqs_filtered, magnitude_filtered
            )
    ...
    def _match_noise_profile(
        self, centroid: float, rolloff: float, bandwidth: float, profile: Dict, freqs: np.ndarray, magnitude: np.ndarray
    ) -> float:
